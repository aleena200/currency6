...
def predict_class(img, threshold=0.8):
    processed_img = preprocess_image(img)
    predictions = model.predict(processed_img)[0]  # get probabilities for each class
    max_prob = np.max(predictions)
    class_id = np.argmax(predictions)
    
    if max_prob < threshold:
        return "This is not a currency note"
    
    # Get the corresponding message from the class_labels dictionary
